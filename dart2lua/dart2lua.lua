...
function parseVariable(cls, type, str, read, write)
    ...
    name = removeKeyWord(name, 'const')
    name = removeKeyWord(name, 'final')
    
    local canRead
    name, canRead = removeKeyWord(name, 'get')
    canRead = canRead or read
    
    local canWrite
    name, canWrite = removeKeyWord(name, 'set')
    canWrite = canWrite or write
...
function beginGenLua()
    ...
    funcBuf:writeln()
    funcBuf:writeln("part of lua_flutter;")
...
function genLua()
    ...
    for _, cls in ipairs(classes) do
        ...
        if #cls.vars == 0 then --vars is empty
            bindBuf:writeln("  luaRegisterClass(L, '%s', '%s', 0);", cls.name, cls.super)
        else
            bindBuf:writeln("  address = ffi.Pointer.fromFunction<lua_CFunction>(_%s_getProp_, exceptionalReturn).address;", cls.name)
            bindBuf:writeln("  luaRegisterClass(L, '%s', '%s', address);", cls.name, cls.super)
